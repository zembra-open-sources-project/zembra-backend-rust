...
use crate::repositories::taxonomy::get_or_create_tag_in_transaction;
...
impl NotesRepository {
    ...
    pub async fn list_note_tags(
        ...
        self.list_note_tags_by_id(workspace_id, &note.id).await
    }
    ...
    ///
    /// Returns tag records ordered by name.
    pub async fn list_note_tags_by_id(
        &self,
        workspace_id: &str,
        note_id: &str,
    ) -> Result<Vec<TagRecord>, ApiError> {
    ...
    pub async fn add_tag_to_note(
        ...
        let tag =
            attach_tag_to_note_in_transaction(&mut transaction, workspace_id, &note, tag_name)
                .await?;
    ...
    pub async fn remove_tag_from_note(
        ...
        let note = self.get_note_by_ref(workspace_id, note_ref).await?;
        let mut transaction = self.pool.begin().await?;
        detach_tag_from_note_in_transaction(&mut transaction, workspace_id, &note, tag_name)
            .await?;
        transaction.commit().await?;
...
pub(super) async fn attach_tag_to_note_in_transaction(
    ...
    let tag = get_or_create_tag_in_transaction(transaction, workspace_id, tag_name).await?;
    ...
    if result.rows_affected() > 0 {
        record_sync_change_in_transaction(
            ...
            SyncChangeInput {
                ...
                payload: note_tag_payload(workspace_id, &note.id, &tag.id),
            },
        )
        .await?;
    }
        ...
        if result.rows_affected() > 0 {
            record_sync_change_in_transaction(
                ...
                SyncChangeInput {
                    ...
                    base_revision_id: None,
                    new_revision_id: None,
                    payload: note_tag_payload(workspace_id, &note.id, &tag_id),
                },
            )
            .await?;
        }
...
pub(super) async fn replace_note_tags_in_transaction(
    ...
    for tag_name in tag_names {
        target_tags
            .push(get_or_create_tag_in_transaction(transaction, workspace_id, tag_name).await?);
    }
        ...
        if !current_ids.contains(&tag.id) {
            ...
            record_sync_change_in_transaction(
                ...
                SyncChangeInput {
                    ...
                    base_revision_id: None,
                    new_revision_id: None,
                    payload: note_tag_payload(workspace_id, note_id, &tag.id),
                },
            )
            .await?;
        }
