...
/// # Arguments
///
/// * `workspace_id` - Workspace identifier for the note snapshot.
/// * `note` - Note record to serialize.
///
/// # Returns
///
/// Returns a JSON payload containing the workspace-scoped note snapshot.
pub(super) fn note_payload(workspace_id: &str, note: &NoteRecord) -> serde_json::Value {
    serde_json::json!({
        "id": note.id,
...
/// # Arguments
///
/// * `workspace_id` - Workspace identifier for the relation.
/// * `note_id` - Note identifier.
/// * `tag_id` - Tag identifier.
///
/// # Returns
...
///
/// Returns a JSON payload for the relation change.
pub(super) fn note_link_payload(workspace_id: &str, link: &NoteLinkRecord) -> serde_json::Value {
    serde_json::json!({
        "id": link.id,
        "workspace_id": workspace_id,
