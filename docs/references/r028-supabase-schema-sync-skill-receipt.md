...
```bash
cd /Users/yat/code/vibeProjects/ZembraProjects/zembra-backend-rust
./scripts/verify_supabase_real_sync.sh
```

r028 已通过的第一验收输出显示，本地和远端 `schema_contract` 都是 `0.5.0`，九张表在同步前后完整行比较全部一致，`/sync/run` 返回 `{"pulled":0,"pushed":0}`，脚本最终输出 `existing-data sync verification passed by full table row comparison`。
