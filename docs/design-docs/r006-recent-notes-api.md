...
新增 `POST /notes/recent`，用于 Web 前端获取最近笔记内容。接口通过 JSON body 传入可选 `limit` 和 `note_uuid`，默认返回最近 50 条，最多返回 100 条，并按 `updated_at DESC` 从新到旧返回未删除、未归档笔记。传入 `note_uuid` 时，以该 note 的 `(updated_at, id)` 为游标返回更旧笔记，不包含游标 note 本身。
...
| 支持向下刷新 | 请求体支持可选 `note_uuid` 游标，方便前端持续加载旧笔记 |
...
| 范围 | 内容 |
| --- | --- |
| DTO | 扩展 recent notes request body，包含 `limit` 和 `note_uuid` 校验 |
| Repository | 扩展查询方法，过滤 `deleted_at` 和 `archived_at`，支持 `(updated_at, id)` 游标 |
...
| Repository | `src/repositories/notes.rs` | 扩展 `list_recent_notes(limit, note_uuid)`，SQL 过滤 `deleted_at IS NULL` 和 `archived_at IS NULL` |
| Service | `src/services/notes.rs` | 扩展 `recent_notes(request)`，调用 repository 并返回 `Vec<NoteRecord>` |
...
| `note_uuid` | string | 否 | 无 | 完整 32 位 hex note ID | 翻页游标，返回该 note 之后更旧的笔记 |
...
| 游标定位 | `note_uuid` 对应的 note 必须未删除、未归档 |
| 游标翻页 | 返回 `updated_at < cursor.updated_at OR (updated_at = cursor.updated_at AND id < cursor.id)` 的记录 |
| 稳定排序 | `ORDER BY updated_at DESC, id DESC` |
...
| 游标翻页 | 传入有效 `note_uuid` 时返回更旧 notes，不包含游标 note |
| 游标不存在 | 传入不存在、已删除或已归档的 `note_uuid` 时返回 `404 record_not_found` |
| 游标格式 | 传入非 32 位 hex `note_uuid` 时返回 `422 validation_error` |
